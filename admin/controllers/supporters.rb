  ...
  post :export, :protect => true do
    ...
    File.open('tmp/sup.csv',"w:utf-8") do |output|
      ...
      @sups.each do |s|
        ...
        line << s.amount.to_s
        line << ''
        line << s.first_name ? s.first_name.gsub("\"", '') : nil
        line << s.last_name ? s.last_name.gsub("\"", '') : nil
        line << s.gender == 1 ? 'm' : 'f'
        ...
        line << s.mobile_phone
        line << s.email
        line << s.address ? s.address.gsub("\"", '') : nil
        line << s.city ? s.city.gsub("\"", '') : nil
        line << s.zip_code
        line << s.occupation ? s.occupation.gsub("\"", '') : nil
